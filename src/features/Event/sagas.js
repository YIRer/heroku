...
/** 데이터 처리를 위한 액션 불러오기 */
import { actions as eventActions } from "./actions";
/** 데이터 처리를 위한 타입 불러오기 */
...
export function* getTypeDefineSaga(action) {
  ...
  try {
    ...
    yield put(eventActions.getTypeDefineSuccess(event));
  } catch (error) {
    yield put(eventActions.getTypeDefineFailure(error));
  }
}
...
export function* watchEventsSaga() {
  yield takeLatest(eventTypes.GET_TPYE_DEFINE_REQUEST, getTypeDefineSaga)
